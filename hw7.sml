...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      (case e of
      	...
      	| Var s => eval_prog(Shift(deltaX,deltaY,eval_prog(e,env)), env)
      	| LineSegment (xstart,ystart,xend,yend) => LineSegment (xstart+deltaX, ystart+deltaY, xend+deltaX, yend+deltaY)
      	| _ => e)
...
fun preprocess_prog e = 
	...
	LineSegment (xstart,ystart,xend,yend) => 
		if real_close(xstart,xend) andalso real_close(ystart,yend)
		then Point(xstart,ystart)
		else if xstart > xend orelse (real_close(xstart,xend) andalso ystart > yend)
